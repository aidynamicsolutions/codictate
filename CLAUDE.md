...
**Tooltip tip (important):** For tooltips on buttons, use `TooltipTrigger asChild` with an `inline-flex` wrapper around the button:
`<TooltipTrigger asChild><span className="inline-flex"><Button ... /></span></TooltipTrigger>`.
This avoids hover/focus issues, especially when buttons can become disabled (`disabled:pointer-events-none`).
...
Codictate supports command-line parameters on all platforms for integration with scripts, window managers, and autostart configurations.
