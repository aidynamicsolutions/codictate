...
import { isDuplicateEntry } from "./dictionaryUtils";
import type { CustomWordEntry } from "@/bindings";

// Helper to create a CustomWordEntry for tests
function createEntry(input: string, replacement?: string, isReplacement = false): CustomWordEntry {
  return {
    input,
...
describe("isDuplicateEntry", () => {
  ...
  describe("edge cases", () => {
    ...
    it("should handle large number of entries", () => {
      const entries = Array.from({ length: 1000 }, (_, i) => createEntry(`word${i}`));
