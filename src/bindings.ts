...
async getUserDictionary() : Promise<Result<CustomWordEntry[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_user_dictionary") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async setUserDictionary(entries: CustomWordEntry[]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("set_user_dictionary", { entries }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
